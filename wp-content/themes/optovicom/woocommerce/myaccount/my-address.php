...
if (!wc_ship_to_billing_address_only() && wc_shipping_enabled()) {
    $get_addresses = apply_filters('woocommerce_my_account_get_addresses', array(
        'billing'  => __('Основной адрес', 'woocommerce'),
        'shipping' => __('Дополнительный адрес', 'woocommerce'),
    ), $customer_id);
} else {
    $get_addresses = apply_filters('woocommerce_my_account_get_addresses', array(
        'billing' => __('Основной адрес', 'woocommerce'),
...
?>
  <h2 class="account-page__title">
      <?php echo apply_filters('woocommerce_my_account_my_address_description', __('Адреса доставки', 'woocommerce')); ?>
  </h2>
